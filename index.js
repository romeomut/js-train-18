...
function checkData(data) {
  ...
  try {
    if (Object.keys(data).length === 0) {
      throw new Error(`Об'єкт пустий`, {cause: `Об'єкт пустий`})
    }else{
      return data
    }
  } catch (error) {
...
function getBookById(books, id) {
  ...
  try {
    let result = books.find((e) => e['id'] === id)
    if (!result) {
      let err = new TypeError(`Книга з ID ${id} не знайдена!`)
      throw err
    }else{
      return `Книга: ${result['title']}`
    }
  } catch (error) {
...
function findEvenNumber(numbers) {
  ...
  let evenNumber
  evenNumber = numbers.find((e) => e % 2 === 0)
...
function validateUser(user) {
  ...
  let isObject = {
    ...
    error: `Об'єкт користувача не вказано!`
  }

  let isName = {
    exam: function (){
      return Object.hasOwn(user, 'name')
    },
    error: `Об'єкт користувача не вказано!`
  }
  ...
  let isEmail = {
    exam: function (){
      return Object.hasOwn(user, 'email')
    },
    error: `Email користувача не вказано!`
  }

  let arrExam = [isObject, isName, isEmail]

  try {
    arrExam.forEach((e) => {
    if (!e.exam()) {
      throw new Error(e.error, {cause: user})
    }})
  ...
  //Можна було набагато простіше написати... але сенс, якщо можна кайфанути
